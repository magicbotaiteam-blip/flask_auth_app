...
@app.route("/bot/group/<int:bot_id>")
def bot_group(bot_id):
    if "user_id" not in session:
        return redirect(url_for("signin"))
    ...
    if not bot:
        conn.close()
        flash("You must be the owner to manage group members.")
        return redirect(url_for("my_bots"))
    ...
    return render_template("bot_group.html", bot=bot, group_members=group_members, 
                          all_users=all_users, username=session.get("username"))


@app.route("/api/bot/<int:bot_id>/invite", methods=["POST"])
def invite_group_member(bot_id):
    if "user_id" not in session:
    ...
    if not bot:
        conn.close()
        return jsonify({"error": "You must be the owner to invite group members"}), 403
    ...
    if existing:
        conn.close()
        return jsonify({"error": "User already invited or is already a group member"}), 400
    
    # Create invitation
    conn.execute("""
        INSERT INTO group_members (bot_id, user_id, role, invited_by, status)
        VALUES (?, ?, ?, ?, 'pending')
