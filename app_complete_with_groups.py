...
def load_bot_runs_from_s3():
    ...
    try:
        ...
        import logging
        logging.warning(f"Failed to load bot runs from S3: {e}")
        return []


@app.route("/activity")
def bot_activity():
    """Bot activity log — shows a timeline of bot runs from the local cron system"""
    user_id = session.get("user_id")
    role = session.get("role", "customer")

    # Load runs from S3
    all_runs = load_bot_runs_from_s3()

    # Get user's bots for mapping
    conn = get_db_connection()
    cursor = conn.cursor()
    if role == "admin":
        cursor.execute("SELECT id, name FROM bots ORDER BY name")
    ...
    # Build bot name lookup (agent_name -> friendly name)
    bot_name_map = {}
    for bot in user_bots:
        bot_name = bot["name"]
        bot_name_lower = bot_name.lower()
        ...
        for c in candidates:
            bot_name_map[c] = bot_name

    # Known friendly bot names (from extract-bot-runs.py detection)
    friendly_names = {
        "weather_bot": "☀️ Weather & Flight",
        "cruise_danube": "🚢 Viking Danube Cruise",
        "cruise_mediterranean": "🚢 Mediterranean Cruise",
        "stock_tracker": "📈 Stock Tracker",
        "flight_taiwan": "✈️ Washington→Taiwan Flight",
        "flight_cairo": "✈️ Washington→Cairo Flight",
        "church_deals": "🛒 Church Deals",
    ...
    for r in all_runs:
        ...
        if not display_name:
            display_name = bot_name_map.get(agent, "")
        if not display_name:
            display_name = bot_name_map.get(source, "")
        if not display_name:
    ...
    # Get unique run types for filter
    run_types = sorted(set(
        r.get("agent", "") for r in activity
        if r.get("trigger") in ("cron", "user")
    ))
