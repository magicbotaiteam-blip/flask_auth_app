"""
Magic Bot AI - Complete Application with All Planned Enhancements
Main application file integrating all features
"""
...
from analytics import get_analytics, log_bot_event, get_bot_analytics, export_bot_data
...
def init_db():
    ...
    # Create other tables (they might already exist from analytics module)
    conn.execute("""
...
def bot_owner_required(f):
    ...
    def decorated_function(bot_id, *args, **kwargs):
        ...
        if not bot:
            flash("You don't have permission to access this bot.")
...
def index():
    ...
    analytics = get_analytics(str(DB_FILENAME))
    user_stats = analytics.get_user_analytics(session["user_id"], days=30)
...
def my_bots():
    ...
    bots = conn.execute("""
        SELECT b.*, 
               ...
               (SELECT COUNT(*) FROM bot_analytics ba 
                WHERE ba.bot_id = b.id AND ba.timestamp > datetime('now', '-7 days')) as weekly_activity
        FROM bots b
...
def test_bot_api(bot_id):
    ...
    bot = conn.execute("""
        ...
        WHERE id = ? AND (user_id = ? OR id IN (
            SELECT bot_id FROM group_members 
    ...
    if not bot:
        conn.close()
        return jsonify({"error": "Bot not found or no access"}), 404
...
@app.route("/api/bot/<int:bot_id>/send", methods=["POST"])
@login_required
def send_bot_api_message(bot_id):
    ...
    bot = conn.execute("""
        ...
        WHERE id = ? AND (user_id = ? OR id IN (
            SELECT bot_id FROM group_members 
            WHERE user_id = ? AND status = 'accepted' AND role IN ('owner', 'admin')
        ))
    """, (bot_id, user_id, user_id)).fetchone()
    
    if not bot:
        conn.close()
        return jsonify({"error": "Bot not found or no permission"}), 403
    ...
    if result.get('success'):
        log_bot_event(bot_id, 'message_sent', {
            'recipient': data['recipient'],
            'message_length': len(data['message']),
            'platform': bot['messaging']
        })
