"""
Magic Bot AI - Working Enhanced Application
A complete, working version with all enhancements
"""
...
def init_db_safe():
    ...
    # Enhanced tables (create if doesn't exist, ignore errors)
    try:
        conn.execute("""
...
def index():
    ...
    # Get user stats if analytics available
    user_stats = {}
    if HAS_ENHANCEMENTS:
        try:
...
def my_bots():
    ...
    # Get user's bots with enhanced data if available
    if HAS_ENHANCEMENTS:
        bots = conn.execute("""
            SELECT b.*, 
...
@app.route("/api/bot/<int:bot_id>/analytics")
@login_required
def get_bot_analytics_api(bot_id):
    """Get bot analytics"""
    if not HAS_ENHANCEMENTS:
        return jsonify({"error": "Enhanced features not available"}), 501
    ...
    try:
        analytics = get_analytics(str(DB_FILENAME))
        stats = analytics.get_event_summary(bot_id, days=7)
...
def log_bot_event_api(bot_id):
    ...
    # Log event
    try:
        success = log_bot_event(bot_id, data['event_type'], data.get('event_data'))
        if success:
...
if __name__ == "__main__":
    print("=" * 60)
    print("Magic Bot AI - Enhanced Flask Application")
    print("=" * 60)
